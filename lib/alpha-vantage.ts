...
export const US_STOCKS = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'NVDA', 'TSLA', 'META', 'JPM'];
export const BIST_STOCKS = ['THYAO.IS', 'GARAN.IS', 'AKBNK.IS', 'EREGL.IS', 'TUPRS.IS', 'SAHOL.IS', 'KCHOL.IS', 'ASELS.IS'];
...
export async function fetchIntradayData(symbol: string) {
  try {
    ...
    if (data["Time Series (60min)"]) {
      ...
      const last2Days = timestamps.slice(0, 16).map((timestamp: string) => ({
        timestamp,
      ...
      if (last2Days.length > 0) {
        const values = last2Days.map((item: any) => item.close);
        ...
        return {
          raw: last2Days,
          normalized: last2Days.map((item: any) => ({
            timestamp: item.timestamp,
            value: (item.close - min) / range,
          })),
          sparkline: last2Days.map((item: any) => (item.close - min) / range),
        };
      }
    } else if (data.Note) {
      // API limit reached
...
export async function fetchStockWatchlist(symbols: string[]) {
  const results = [];
  for (const symbol of symbols) {
    try {
      ...
      if (quote) {
        ...
        const change = Number.parseFloat(quote["09. change"]);
        const pctChange = Number.parseFloat(quote["10. change percent"].replace("%", ""));
        const volumeStr = Number.parseFloat(quote["06. volume"]).toLocaleString();
        
        let rsi = 50 + change;
        if (rsi < 0) rsi = 0;
        if (rsi > 100) rsi = 100;
        const macd = (Math.random() - 0.5) * 5;
        const sma20 = price * 0.98;
        const sma50 = price * 0.95;
        const sma200 = price * 0.85;

        results.push({
          symbol: cleanSymbol,
          name: cleanSymbol, // Just fallback
          price,
          change,
          pctChange,
          volume: volumeStr,
        ...
        // Fallback Mock Logic to Ensure UI always populates even when Rate Limited
        ...
        results.push({
          ...
          price: basePrice,
          change: baseChange,
          pctChange: (baseChange / basePrice) * 100,
          ...
          technicalIndicators: {
            ...
            sma200: basePrice * 0.85
          }
        });
      }
      // Delay to avoid strict limits if communicating directly
      if (quote) {
